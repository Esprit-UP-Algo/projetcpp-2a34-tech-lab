...
{
    ui->stackedWidget->setCurrentWidget(ui->Gestion_equipements);
    load_equipement_data();
}
...
void MainWindow::on_rech_equipement_btn_clicked()
{
    ...
    QString valeur = ui->rech_equipement_input->currentText().trimmed();
    ...
    if (critere == "ID") {
        ...
        if (!ok || ref <= 0) {
            ...
            ui->rech_equipement_input->clear();
            ui->rech_equipement_input->setFocus();
            return;
        }
    }
...
{
    QString critere = ui->LineEdit_rech_formateur->text().trimmed();;
...
void MainWindow::initialiserUIAtelier()
{
    ...
    // Remplir la comboBox des jours
    QStringList jours = {"Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi", "Samedi", "Dimanche"};
    ui->jour_atelier->clear();
    ui->jour_atelier->addItems(jours);

    // Configurer les spinboxes
    ui->capacite_atelier->setRange(1, 1000);
    ui->duree_atelier->setRange(1, 24);
    ui->salle_atelier->setRange(1, 999);
...
void MainWindow::on_ajouter_atelier_btn_clicked()
{
    ...
    int capacite = ui->capacite_atelier->value();
    QString jour = ui->jour_atelier->text().trimmed();();
    QString horaire = ui->horaire_atelier->text();
    int duree = ui->duree_atelier->value();
    int salle = ui->salle_atelier->value();
...
void MainWindow::on_modifier_atelier_btn_clicked()
{
    ...
    QString nom = ui->nom_atelier->text().trimmed();
    int capacite = ui->capacite_atelier->value();
    QString jour = ui->jour_atelier->currentText();
    QString horaire = ui->horaire_atelier->text();
    int duree = ui->duree_atelier->value();
    int salle = ui->salle_atelier->value();
...
void MainWindow::on_tableView_ateliers_clicked(const QModelIndex &index)
{
    ...
    ui->capacite_atelier->setValue(record.value("Capacité").toInt());
    ui->jour_atelier->setCurrentText(record.value("Jour").toString());
    ui->duree_atelier->setValue(record.value("Durée").toInt());
    ui->salle_atelier->setValue(record.value("Numéro de salle").toInt());

    // Gestion de l'horaire
    QString horaire = record.value("Horaire").toString();
    QTime time = QTime::fromString(horaire, "hh:mm");
    if (time.isValid()) {
        ui->horaire_atelier->setTime(time);
    }
}
...
void MainWindow::viderFormulaireAtelier()
{
    ...
    ui->capacite_atelier->setValue(1);
    ui->jour_atelier->setCurrentIndex(0);
    ui->horaire_atelier->setTime(QTime(8, 0));
    ui->duree_atelier->setValue(1);
    ui->salle_atelier->setValue(1);
}
