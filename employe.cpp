...
{
    QSqlQuery query;
    query.prepare("INSERT INTO employes (id, nom, prenom, email, telephone, poste,mdp ,etat) "
                  "VALUES (:id, :nom, :prenom, :email, :telephone, :poste, :mdp, :etat)");
...
bool Employe::findemp(QString id)
{
    bool ok;
    ...
    QSqlQuery q;
    q.prepare("SELECT id FROM employes WHERE id = :id LIMIT 1");  // table name matches QSqlTableModel
    q.bindValue(":id", idInt);

    if (!q.exec()) {
        qDebug() << "Erreur SQL findemp:" << q.lastError();
        return false;
    }

    return q.next();  // true if ID exists
}
