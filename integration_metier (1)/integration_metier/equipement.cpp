...
//stat
QMap<QString, double> Equipement::statistiques(QString critere)
{
    ...
    // 1️⃣ Récupérer le total
    int total = 0;
    if (query.exec("SELECT COUNT(*) FROM EQUIPEMENTS")) {
        query.next();
    ...
    // 2️⃣ Répartition par catégorie
    query.prepare("SELECT CATEGORIE, COUNT(*) FROM EQUIPEMENTS GROUP BY CATEGORIE");
    if (!query.exec())
        return stats;
