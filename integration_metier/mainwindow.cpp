...
// Pour les graphiques (stats pie/bar)
#include <QChart>
#include <QChartView>
#include <QPieSeries>
#include <QPieSlice>
...
// Pour le calendrier et dialogues
#include <QDialog>
#include <QVBoxLayout>
#include <QHBoxLayout>
#include <QLabel>
...
// Pour PDF + extraction texte (upload CV)
...
MainWindow::MainWindow(QWidget *parent)
    ...
    , seuilStockMin(15)  // Seuil par défaut de 15 cm (à ajuster selon les besoins)
{
    ui->setupUi(this);
connect(&Ar, &Arduino::dataReceived, this, &MainWindow::arduino_data_received);
    // --- Connexion des signaux ---
    connect(&Ar, &Arduino::distanceMeasured, this, &MainWindow::onDistanceMeasured);
    connect(&Ar, &Arduino::dataReceived, this, [](const QString &line){
        qDebug() << "Arduino data:" << line;
    });
    connect(&Ar, &Arduino::erreurConnexion, this, [](const QString &msg){
        qDebug() << "Erreur Arduino:" << msg;
    });

    // --- Connexion au port Arduino ---
    Ar.connect_arduino("COM5"); // ou vide pour auto-detection

    // --- Initialisation variables ---
    seuilStockMin = 20;           // seuil par défaut
    monitoredReference = 1;       // référence de l’équipement à surveiller
    // ======================== ATELIERS ========================
    ...
    ui->lineEdit_ref->setValidator(new QIntValidator(100, 999999999, this)); // Réduit à la valeur maximale d'un int signé 32 bits
...
{
    QString critere = "categorie";  // 🔥 stats uniquement par catégorie
...
{
    QString critere = "categorie";  // 🔥 tu as demandé SEULEMENT catégorie
    ...
    // ⚠️ OBLIGATOIRE : utiliser QChartView
    QChartView *view = new QChartView(chart);
    view->setRenderHint(QPainter::Antialiasing);
...
{
    QString texte = ui->lineEdit_recherche->text().trimmed();
    ui->TableWidget->clearContents();
    ui->TableWidget->setRowCount(0);
    ...
    QString sql;
    if (texte.isEmpty()) {
        sql = "SELECT * FROM ATELIER ORDER BY REFERENCE";
    } else {
        sql = "SELECT * FROM ATELIER WHERE REFERENCE LIKE ? OR UPPER(NOM_ATELIER) LIKE UPPER(?) OR NUM_SALLE = ? ORDER BY REFERENCE";
        query.prepare(sql);
        query.addBindValue("%" + texte + "%");
        query.addBindValue("%" + texte + "%");
        query.addBindValue(texte.toInt());
    }
    if (!query.exec(sql)) { QMessageBox::critical(this, "Erreur", query.lastError().text()); return; }
...
void MainWindow::on_btnExporterPDF_clicked()
{
    ...
    // === EMAIL prend toute la place restante ===
    int totalFixed = 0;
    int emailCol = -1;
    ...
    // 🔥 Fonction locale pour dessiner l’en-tête
    auto drawHeader = [&]() {
...
{
    QMap<QString, int> stats = Atmp.statistiquesJour(); 
...
void MainWindow::onDistanceMeasured(int etat)
{
    QString newStockValue = (etat == 0) ? "suffisant" : "insuffisant";

    QSqlQuery q;
    q.prepare("UPDATE EQUIPEMENTS SET STOCK = :s WHERE REFERENCE = :r");
    q.bindValue(":s", newStockValue);
    q.bindValue(":r", monitoredReference);
    q.exec();
    ...
    if (etat == 1) {
        ...
        msg->setWindowTitle("Stock bas");
        msg->setText("Veuillez restocker cet équipement !");
        msg->setStandardButtons(QMessageBox::Ok);
        msg->setModal(false);   // <--- Important
        msg->show();

        // Envoi immédiat au buzzer
        Ar.envoyerCommande("BUZZER_ON");
    }
    else {
        Ar.envoyerCommande("BUZZER_OFF");
    }
}

void MainWindow::arduino_data_received(const QString &line)
{
    QString data = line.trimmed(); // supprime \r et \n
    bool ok;
    int distance = data.toInt(&ok);

    if(ok) {
        qDebug() << "Distance mesurée:" << distance << "cm";

        if(distance < 20 && distance > 0) {
            // alerter dans l'UI si nécessaire
        }
    }
}
