    ...
    ,modelAteliers(nullptr)
{
    ...
    QSqlQueryModel* modelF = F.afficher();
    ui->tableView_formateurs->setModel(modelF);
    initialiserUIAtelier();
...
void MainWindow::on_rech_equipement_btn_clicked()
{
    ...
    QString valeur = ui->LineEdit_rech_adh_2->text().trimmed();
    ...
    if (critere == "REF") {
        ...
        if (!ok || ref <= 0) {
            ...
            ui->LineEdit_rech_adh_2->clear();
            ui->LineEdit_rech_adh_2->setFocus();
            return;
        }
    }
...
{
    QString critere = ui->comboBox_recherche_formateur->currentText();
...
void MainWindow::initialiserUIAtelier()
{
    // Configuration du tableau des ateliers
    modelAteliers = new QSqlQueryModel(this);
    ui->tableView_ateliers->setModel(modelAteliers);
    ui->tableView_ateliers->setSelectionBehavior(QAbstractItemView::SelectRows);
    ui->tableView_ateliers->setSelectionMode(QAbstractItemView::SingleSelection);

    // Configuration du QDateEdit pour les jours
    ui->jour_atelier->setDisplayFormat("dddd"); // Affiche le nom du jour
    ui->jour_atelier->setDate(QDate::currentDate());
    ui->jour_atelier->setCalendarPopup(true);

    // Configurer les champs de saisie avec des validateurs
    QIntValidator *capaciteValidator = new QIntValidator(1, 1000, this);
    ui->capacite_atelier->setValidator(capaciteValidator);
    
    QIntValidator *dureeValidator = new QIntValidator(1, 24, this);
    ui->duree_atelier->setValidator(dureeValidator);
    
    QIntValidator *salleValidator = new QIntValidator(1, 999, this);
    ui->salle_atelier->setValidator(salleValidator);
    
    // Définir des valeurs par défaut
    ui->capacite_atelier->setText("10");
    ui->duree_atelier->setText("1");
    ui->salle_atelier->setText("1");

    // Charger les données initiales
    on_rafraichir_atelier_btn_clicked();
}

void MainWindow::viderFormulaireAtelier()
{
    ui->ref_atelier->clear();
    ui->nom_atelier->clear();
    ui->capacite_atelier->clear();
    ui->duree_atelier->clear();
    ui->salle_atelier->clear();
    ui->jour_atelier->setDate(QDate::currentDate());
    ui->horaire_atelier->setText("08:00");  // Default time 08:00
...
void MainWindow::on_ajouter_atelier_btn_clicked()
{
    // Récupération des données
    QString reference = ui->ref_atelier->text().trimmed();
    QString nom = ui->nom_atelier->text().trimmed();
    int capacite = ui->capacite_atelier->text().toInt();
    QString jour = ui->jour_atelier->text().trimmed();
    QString horaire = ui->horaire_atelier->text();
    int duree = ui->duree_atelier->text().toInt();
    int salle = ui->salle_atelier->text().toInt();

    // Validation
    if (!Atelier::referenceValide(reference)) {
        QMessageBox::warning(this, "Erreur", "Référence invalide ! Maximum 10 caractères alphanumériques.");
        return;
    }

    if (Atelier::referenceExiste(reference)) {
        QMessageBox::warning(this, "Erreur", "Cette référence existe déjà !");
        return;
    }

    if (nom.isEmpty()) {
        QMessageBox::warning(this, "Erreur", "Le nom de l'atelier est obligatoire !");
        return;
    }

    // Création et ajout
    Atelier nouvelAtelier(reference, nom, capacite, jour, horaire, duree, salle);

    if (nouvelAtelier.ajouter()) {
        QMessageBox::information(this, "Succès", "Atelier ajouté avec succès !");
        viderFormulaireAtelier();
        on_rafraichir_atelier_btn_clicked();
    } else {
        QMessageBox::critical(this, "Erreur", "Erreur lors de l'ajout de l'atelier !");
    }
}

void MainWindow::on_modifier_atelier_btn_clicked()
{
    QString reference = ui->ref_atelier->text().trimmed();

    if (reference.isEmpty()) {
        QMessageBox::warning(this, "Erreur", "Aucun atelier sélectionné !");
        return;
    }
    ...
    // Modification
    Atelier atelier(reference, nom, capacite, jour, horaire, duree, salle);

    if (atelier.modifier()) {
        QMessageBox::information(this, "Succès", "Atelier modifié avec succès !");
        on_rafraichir_atelier_btn_clicked();
    } else {
...
void MainWindow::on_supprimer_atelier_btn_clicked()
{
    QString reference = ui->ref_atelier->text().trimmed();
    ...
    QMessageBox::StandardButton confirmation = QMessageBox::question(
        this, "Confirmation",
        QString("Voulez-vous vraiment supprimer l'atelier %1 ?").arg(reference),
        QMessageBox::Yes | QMessageBox::No
        );

    if (confirmation == QMessageBox::Yes) {
        if (Atelier().supprimer(reference)) {
            QMessageBox::information(this, "Succès", "Atelier supprimé avec succès !");
            viderFormulaireAtelier();
            on_rafraichir_atelier_btn_clicked();
        } else {
...
void MainWindow::on_rafraichir_atelier_btn_clicked()
{
    if (modelAteliers) {
        QSqlQueryModel *nouveauModel = Atelier().afficher();
        if (nouveauModel) {
            delete modelAteliers;
            modelAteliers = nouveauModel;
            ui->tableView_ateliers->setModel(modelAteliers);
            ui->tableView_ateliers->resizeColumnsToContents();
        }
    }
}

void MainWindow::on_rech_atelier_btn_clicked()
{
    QStringList criteres = {"Référence", "Nom", "Salle"};
    QString critere = QInputDialog::getItem(this, "Rechercher un atelier", "Critère de recherche:", criteres, 0, false);

    if (critere.isEmpty()) return;

    QString valeur = QInputDialog::getText(this, "Rechercher un atelier", "Valeur recherchée:");

    if (valeur.isEmpty()) return;

    QSqlQueryModel *modelRecherche = Atelier().rechercher(critere, valeur);

    if (modelRecherche) {
        ui->tableView_ateliers->setModel(modelRecherche);
        ui->tableView_ateliers->resizeColumnsToContents();
...
void MainWindow::on_tri_up_atelier_clicked()
{
    QStringList criteres = {"Référence", "Capacité", "Jour"};
    QString critere = QInputDialog::getItem(this, "Trier les ateliers", "Trier par:", criteres, 0, false);

    if (!critere.isEmpty()) {
        QString ordre = "ASC";
        QSqlQueryModel *modelTrie = Atelier().trier(critere.toLower(), ordre);
        if (modelTrie) {
            ui->tableView_ateliers->setModel(modelTrie);
            ui->tableView_ateliers->resizeColumnsToContents();
        }
    }
}

void MainWindow::on_tri_down_atelier_clicked()
{
    QStringList criteres = {"Référence", "Capacité", "Jour"};
    QString critere = QInputDialog::getItem(this, "Trier les ateliers", "Trier par:", criteres, 0, false);
...
void MainWindow::on_stats_atelier_btn_clicked()
{
    QMap<QString, int> stats = Atelier().statistiquesParJour();

    QString message = "📊 Statistiques des ateliers par jour :\n\n";
    int total = 0;

    for (auto it = stats.begin(); it != stats.end(); ++it) {
        message += QString("• %1 : %2 atelier(s)\n").arg(it.key()).arg(it.value());
        total += it.value();
    }

    message += QString("\n📈 Total : %1 atelier(s)").arg(total);

    QMessageBox::information(this, "Statistiques des ateliers", message);
}

void MainWindow::on_tableView_ateliers_clicked(const QModelIndex &index)
{
    if (!index.isValid()) return;

    int row = index.row();
    QSqlRecord record = modelAteliers->record(row);

    ui->ref_atelier->setText(record.value("Référence").toString());
    ui->nom_atelier->setText(record.value("Nom d'atelier").toString());
    ui->capacite_atelier->setText(record.value("Capacité").toString());
    ui->jour_atelier->setDate(QDate::fromString(record.value("Jour").toString(), "dddd"));
    ui->duree_atelier->setText(record.value("Durée").toString());
    ui->salle_atelier->setText(record.value("Numéro de salle").toString());

    // Gestion de l'horaire
    QString horaire = record.value("Horaire").toString();
    // Vérifier si l'horaire est au format valide (HH:MM)
    QTime time = QTime::fromString(horaire, "hh:mm");
    if (time.isValid()) {
        ui->horaire_atelier->setText(time.toString("hh:mm"));
    } else {
        // Si le format n'est pas valide, utiliser une valeur par défaut
        ui->horaire_atelier->setText("08:00");
    }
}

// AJOUTEZ CETTE METHODE DE NAVIGATION
void MainWindow::on_ateliers_clicked()
{
    ui->stackedWidget->setCurrentWidget(ui->page_ateliers);
    on_rafraichir_atelier_btn_clicked(); // Rafraîchir à chaque accès
}
