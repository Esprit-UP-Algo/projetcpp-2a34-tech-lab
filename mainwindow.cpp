    ...
    , model(nullptr)
{
    ...
    connect(ui->btnFonctionnalite, &QPushButton::clicked,
    ...
    switch (ret) {
    case 0 :
        qDebug()<<"Arduino is available and connected to : "<<ard.getarduino_port_name();
        break;

    case 1 :
        qDebug()<<"Arduino is available but not connected to : "<<ard.getarduino_port_name();
        break;
    case -1 :
    ...
    connect(timer, &QTimer::timeout, this, &MainWindow::readArduinoData);
    timer->start(50);   // check serial every 50 ms
...
{
    QByteArray raw = ard.read_from_arduino();
    if (raw.isEmpty()) return;

    // Buffer for partial serial chunks
    ...
    {
        QString data = buffer.left(newlineIndex).trimmed();
        ...
        // Only process complete lines starting with KEY:
        if (!data.startsWith("KEY:"))
            continue;

        QString key = data.mid(4).trimmed();

        // Confirm button 'C': final UID
        if (key == "C")
        {
            if (uid.length() == 8)
            {
                qDebug() << "Full 8-digit UID received:" << uid;

                Employe e;
                bool found = e.findemp(uid);
                qDebug() << "Found:" << found;

                QString msgToArduino;

                if (found)
                {
                    // Retrieve employee name from database
                    QSqlQuery q;
                    q.prepare("SELECT nom || ' ' || prenom FROM employes WHERE id = :id LIMIT 1");
                    q.bindValue(":id", uid.toInt());

                    if (q.exec() && q.next())
                    {
                        QString fullName = q.value(0).toString();
                        qDebug() << "Employee name:" << fullName;
                        msgToArduino = "NOM:" + fullName;
                    }
                    else
                    {
                        qDebug() << "Failed to retrieve name from DB:" << q.lastError();
                        msgToArduino = "NOM:NOT_FOUND";
                    }
                }
                else
                {
                ...
                ard.write_to_arduino(msgToArduino.toUtf8());
            }
            else
            {
                qDebug() << "Incomplete UID:" << uid;
            }

            uid.clear();  // reset for next UID
            continue;
        }
...
void MainWindow::on_btnAjouter_clicked()
{
    ...
    // 2️⃣ Construct an Employe object
    Employe e;
    e.setId(id);
    e.setNom(ui->lineEditNom->text());
    e.setPrenom(ui->lineEditPrenom->text());
    e.setEmail(ui->lineEditEmail->text());
    e.setTelephone(ui->lineEditTelephone->text());
    e.setPoste(ui->lineEditPoste->text());

    // 3️⃣ Call ajouter() to insert into DB
    if(e.ajouter()) {
        QMessageBox::information(this,"Succès","Employé ajouté !");

        // 4️⃣ Refresh table view
        model->select();
